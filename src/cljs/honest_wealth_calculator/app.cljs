...
(defn calc-roi [base annual-rate years]
  (- (* (.pow js/Math (+ 1 annual-rate) years) base) base))
...
(defn to-own []
  (let [purchase-price (:purchase-price @data)
        ...
        months (* 12 years)
        interest-cost (int (:total-interest-cost result))
        monthly-interest (/ interest-cost months)
    ...
    [panel-with-heading "If buy (plus investments)..."
      [:form {:class "form-horizontal"}
        ...
        (input-row "Monthly mortgage" [money-input "Monthly mortgage payment" :monthly-payments "" monthly nil true])
        (input-row (str "Total interest cost (over " years " years)") [money-input "Interst cost" :monthly-payments "" interest-cost nil true])
        (input-row (str "Montly interest cost") [money-input "Interst cost" :monthly-interest "" monthly-interest nil true])
        ...
        (input-row "Monthly profit/loss (income - cost)" [money-input "Monthly P&L" :monthly-pnl "" monthly-pnl nil true])
        (input-row "Annual profit/loss" [money-input "Annual total P&L" :ownership-cost "" annual-pnl nil true])
        (input-row (str "Total profit/loss (over " years " years)") [money-input "Annual total P&L" :annual-pnl "" total-pnl nil true])
        (input-row "Total gain from appreciation (divided by inflation)" [money-input "" :total-appreciation-gain "" "TODO" nil true])]]))
...
(defn to-rent []
  (let [result (calc-monthly-payment)
        ...
        total-pnl (- total-roi total-rent)]
    [panel-with-heading "If rent (plus investments)..."
        [:form {:class "form-horizontal"}
          ...
          (input-row "Return rate of alternative investment" [percentage-input "" :investment-return-rate "" investment-return-rate (update-val-fn :investment-return-rate) false])
          ...
          (input-row "Total profit & loss" [money-input "" :total-pnl "" total-pnl nil true])
          ]]))
